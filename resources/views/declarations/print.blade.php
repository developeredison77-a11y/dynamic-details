...
<head>
    ...
    <title>Asset Declaration {{ $declaration->declaration_number }}</title>
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .document-number { min-width: 210px; border: 1px solid #d8e0ec; border-radius: 8px; background: #f8fafc; padding: 14px; text-align: right; }
        .document-number strong { display: block; margin-top: 5px; font-size: 18px; }
        ...
        .terms { color: #334155; line-height: 1.8; white-space: pre-line; }
...
<body onload="window.print()">
    ...
    <main class="document">
        <header class="document-header">
            <div>
                <span class="eyebrow">Asset Declaration</span>
                <h1>{{ $declaration->assignment?->asset?->asset_tag }} to {{ $declaration->assignment?->employee?->name_en }}</h1>
            </div>
            <div class="document-number">
                <span class="eyebrow">Declaration No.</span>
                <strong>{{ $declaration->declaration_number }}</strong>
            </div>
        </header>

        <section class="summary-strip">
            <div><span>Issued Date</span><strong>{{ $declaration->issued_at?->format('M d, Y') }}</strong></div>
            <div><span>Handover Date</span><strong>{{ $declaration->assignment?->handover_date?->format('M d, Y') ?? '-' }}</strong></div>
            <div><span>Status</span><strong>{{ $declaration->assignment?->status?->label() ?? '-' }}</strong></div>
        </section>
        ...
        <section class="content">
            <div class="two-column">
                <article class="section">
                    ...
                    <h2>{{ $declaration->assignment?->employee?->name_en }}</h2>
                    <dl>
                        <div><dt>Code</dt><dd>{{ $declaration->assignment?->employee?->employee_code }}</dd></div>
                        <div><dt>Arabic Name</dt><dd dir="rtl">{{ $declaration->assignment?->employee?->name_ar ?: '-' }}</dd></div>
                    </dl>
                </article>

                <article class="section">
                    <span class="section-title">Asset</span>
                    <h2>{{ $declaration->assignment?->asset?->name }}</h2>
                    <dl>
                        <div><dt>Tag</dt><dd>{{ $declaration->assignment?->asset?->asset_tag }}</dd></div>
                        <div><dt>Brand</dt><dd>{{ $declaration->assignment?->asset?->brand?->name ?? '-' }}</dd></div>
                        <div><dt>Category</dt><dd>{{ $declaration->assignment?->asset?->category?->name ?? '-' }}</dd></div>
                        <div><dt>Serial</dt><dd>{{ $declaration->assignment?->asset?->serial_number ?: '-' }}</dd></div>
                    </dl>
                </article>
            </div>

            <article class="section">
                <span class="section-title">Declaration Terms</span>
                <p class="terms">{{ $declaration->terms }}</p>
            </article>

            <div class="signature-grid">
                <div class="signature-box"><div class="signature-line">Employee Signature</div></div>
                <div class="signature-box"><div class="signature-line">Company Representative</div></div>
            </div>
        </section>
    </main>
</body>
