...
@section('content')
    <section class="dashboard-panel asset-handover-form-panel">
        ...
        <form class="settings-form" method="POST" action="{{ $assignment->exists ? route('asset-handovers.update', $assignment) : route('asset-handovers.store') }}">
            ...
            <div class="form-grid">
                <label class="form-field"><span>Employee</span><select name="employee_id"><option value="">Select employee</option>@foreach($employees as $employee)<option value="{{ $employee->id }}" @selected((string) old('employee_id', $assignment->employee_id) === (string) $employee->id)>{{ $employee->employee_code }}{{ $employee->eid ? ' / EID: '.$employee->eid : '' }} - {{ $employee->name_en }}</option>@endforeach</select>@error('employee_id')<small>{{ $message }}</small>@enderror</label>
