    ...
    <section class="dashboard-panel">
        ...
        <form class="settings-form" method="POST" action="{{ $employee->exists ? route('employees.update', $employee) : route('employees.store') }}">
            ...
            <div class="form-grid">
                ...
                <label class="form-field"><span>Password{{ $employee->exists ? '' : ' *' }}</span><input name="password" type="password" autocomplete="new-password" placeholder="{{ $employee->exists ? 'Leave blank to keep current password' : 'Create a secure password' }}">@error('password')<small>{{ $message }}</small>@enderror</label>
                <label class="form-field"><span>Confirm Password{{ $employee->exists ? '' : ' *' }}</span><input name="password_confirmation" type="password" autocomplete="new-password" placeholder="Repeat password">@error('password_confirmation')<small>{{ $message }}</small>@enderror</label>
