...
@section('content')
    ...
    <section class="dashboard-panel">
        ...
        <form class="settings-form" method="POST" action="{{ route('imports.employees') }}" enctype="multipart/form-data">
            ...
            @endif
            <div class="import-help"><span>Standard imports require name_en, nationality, entity, email, password, role, department, and job title. No password confirmation column is required. You can use IDs or exact active names for role, department, and job title. Legacy EMPM sheets with Name, ID No, Entity, email, Password, and Job Title are also supported: Entity becomes the department, Staff is assigned as the role, and new departments and job titles are added as active master records. Employee codes are generated automatically.</span><a class="btn btn-outline btn-sm" href="{{ route('imports.template', 'employees') }}"><x-dashboard.icon name="download" /> Download Template</a></div>
